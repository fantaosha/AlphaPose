...
class YOLO4TRTDetector(BaseDetector):
    def __init__(self, cfg, opt):
        super(YOLO4TRTDetector, self).__init__()

        self.detector_cfg = cfg
        self.detector_opt = opt
        self.model_cfg = cfg.get(
            'CONFIG', 'detector/yolo4_csp/cfg/yolov4-csp.cfg')
        self.model_weights = cfg.get(
            'WEIGHTS', 'detector/yolo4_csp/weights/yolov4-csp.weights')

        os.environ['YOLO4_HOME'] = os.path.dirname(self.model_weights)

        self.inp_dim = cfg.get('INP_DIM', 512)
    ...
    def image_preprocess(self, img_source):
        ...
        if isinstance(img_source, str):
            img, orig_img, orig_dim_list = prep_image(img_source, self.inp_dim)
        elif isinstance(img_source, torch.Tensor) or isinstance(img_source, np.ndarray):
            img, orig_img, orig_dim_list = prep_frame(img_source, self.inp_dim)
        else:
            raise IOError(
                'Unknown image source type: {}'.format(type(img_source)))
    ...
    def write_results(self, prediction, confidence, num_classes, nms=True, nms_conf=0.5):
        args = self.detector_opt
                ...
                if nms:
                    if platform.system() != 'Windows':
                        ...
                        while image_pred_class.size(0):
                            ...
                            ious = bbox_iou(
                                max_detections[-1], image_pred_class[1:], args)
