...
class YOLODetector(BaseDetector):
    def __init__(self, cfg, opt=None):
        ...
        self.confidence = 0.3 if (False if not hasattr(
            opt, 'tracking') else opt.tracking) else cfg.get('CONFIDENCE', 0.05)
        ...
        if args:
            if len(args.gpus) > 1:
                self.model = torch.nn.DataParallel(
                    self.model, device_ids=args.gpus).to(args.device)
            else:
    ...
    def detect_one_img(self, img_name):
        ...
        with torch.no_grad():
            ...
            img_dim_list = torch.index_select(
                img_dim_list, 0, dets[:, 0].long())
            scaling_factor = torch.min(
                self.inp_dim / img_dim_list, 1)[0].view(-1, 1)
            dets[:, [1, 3]] -= (self.inp_dim - scaling_factor *
            ...
            for i in range(dets.shape[0]):
                dets[i, [1, 3]] = torch.clamp(
                    dets[i, [1, 3]], 0.0, img_dim_list[i, 0])
                dets[i, [2, 4]] = torch.clamp(
                    dets[i, [2, 4]], 0.0, img_dim_list[i, 1])
